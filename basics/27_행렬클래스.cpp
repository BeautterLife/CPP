...
public:
	...
	myMatrix(int nRows, int nCols, const int *data);
	myMatrix(int nRows, int nCols, const int **data);
...
myMatrix::myMatrix(int nRows, int nCols, const int *data) : rows(nRows), cols(nCols), pData(nullptr){
	allocateMemory();
	for(int i=0; i<rows; i++)
		for(int j=0; j<cols; j++)
    ...
    for(int i=0;i<rows;i++){
        for(int j=0;j<mat.cols;j++){
            for(int k=0;k<cols;k++){
                tmp[i][j]+= pData[i][k]*mat.pData[k][j];
            }
        }
    }
    *this = tmp;
...
bool operator==(const myMatrix& a, const myMatrix& b){
    ...
    for(int i=0;i<a.rows;i++){
        for(int j=0;j<a.cols;j++){
            if(a.pData[i][j]!=b.pData[i][j]) return false;
        }
    }
    return true;
}
...
myMatrix myMatrix::operator -() const	{	// unary minus
  // fill in
    myMatrix tmp(rows,cols,0);
    for(int i=0;i<rows;i++){
        for(int j=0;j<cols;j++){
            tmp.pData[i][j]=-pData[i][j];
        }
    }
    return tmp;
}
...
myMatrix& myMatrix::transpose(){
  // fill in
    myMatrix *tmp = new myMatrix(cols,rows,0);
    for(int i=0;i<cols;i++){
        for(int j=0;j<rows;j++){
            tmp->pData[i][j] = pData[j][i];
        }
    }
    this->deleteMemory();
    return *tmp;
